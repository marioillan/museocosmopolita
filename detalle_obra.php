<?php
    ...
    $sql_relacionadas = "SELECT * FROM obras WHERE tematica = '$tematica' AND id != $obra_id LIMIT 4";
    $result_relacionadas = $conn->query($sql_relacionadas);

    $obras_relacionadas = [];
    if ($result_relacionadas->num_rows > 0) {
        while($row = $result_relacionadas->fetch_assoc()) {
            $obras_relacionadas[] = $row;
        }
    }
...
<head>
    ...
    <meta charset="utf-8">
    <title>Museo Cosmopolita - Obra </title>
...
<main>
    <div class="obra">
        <div class="obra-imagen">
            <img src="<?php echo $obra['imagen']; ?>" alt="Imagen Obra">
        </div>

        <div class="descripcion-obra">
            <h2><?php echo $obra['titulo']; ?></h2>
            <p>
                Artista: <?php echo $obra['autor']; ?></a> <br>  
                Cultura: <?php echo $obra['tematica']; ?><br>
                Título: <?php echo $obra['titulo']; ?><br>
                Epoca: <?php echo $obra['epoca']; ?><br>
            </p>
        </div>
    </div>
    ...
    <aside class="obras-relacionadas">
        ...
        <?php foreach ($obras_relacionadas as $relacionada) : ?>
            <div class="obra-recomendada">
                <h3><?php echo $relacionada['titulo']; ?></h3>
                <a href="detalle_obra.php?id=<?php echo $relacionada['id']; ?>"><img src="<?php echo $relacionada['imagen']; ?>" alt="Obra"></a>
                <p>Temática: <?php echo $relacionada['tematica']; ?></p>
            </div>
        <?php endforeach; ?>
    </aside>
